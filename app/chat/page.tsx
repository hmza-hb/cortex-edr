...
function ChatHomeInner() {
    ...
    const load = useCallback(async (preferredThreadId?: string | null) => {
        ...
        try {
            ...
            if (res.redirected || (contentType && !contentType.includes("application/json"))) {
                setLoading(false);
                setMessages((prev) =>
                    prev.length
                        ? prev
                        : [{ role: "assistant", content: "Your session expired or a server error occurred. Please sign in again." }]
                );
                return;
            }
    ...
    const replayFromMessage = async (messageId: string, editedContent?: string) => {
        ...
        const contentType = res.headers.get("content-type");
        if (res.redirected || (contentType && !contentType.includes("application/json"))) {
            throw new Error("Your session expired or a server error occurred. Please sign in again.");
        }
        const data = await res.json();
        if (res.status === 401) {
